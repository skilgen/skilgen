...
async def _skill_half_life_available(db: AsyncSession) -> bool:
    try:
        await db.execute(text("SELECT 1 FROM skill_half_lives LIMIT 1"))
...
async def drift(
    ...
    try:
        ...
        return V8DriftResponse(events=[], total=0)
    events = [
        V8DriftEvent(
            skill_id=skill.id,

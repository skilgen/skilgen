...
def _tier2_commit_trailer(commits: list[Commit]) -> AttributionDecision | None:
    for commit in commits:
        ...
        # or "Generated with [Claude Code](https://claude.ai/claude-code)"
        if re.search(r"co-authored-by:\s*claude", message, re.IGNORECASE):
            agent = "claude_code"
        elif re.search(r"noreply@anthropic\.com", message, re.IGNORECASE):
            agent = "claude_code"
        elif re.search(r"generated with.*claude", message, re.IGNORECASE):
            agent = "claude_code"
        elif re.search(r"co-authored-by:\s*github copilot", message, re.IGNORECASE):
            agent = "copilot"
        # Codex CLI: "Co-Authored-By: Codex" or "Generated with [OpenAI Codex]"
        elif re.search(r"co-authored-by:\s*codex", message, re.IGNORECASE):
            agent = "codex"
        elif re.search(r"generated with.*codex|generated by.*codex", message, re.IGNORECASE):
            agent = "codex"
...
def _tier4_branch_name(pr: PullRequest) -> AttributionDecision | None:
    ...
    for keyword, agent in (("claude", "claude_code"), ("codex", "codex"), ("cursor", "cursor"), ("devin", "devin")):
        if (
            branch.startswith(f"{keyword}/")
            or branch.startswith(f"{keyword}-")
            or f"/{keyword}/" in branch
            or f"-{keyword}-" in branch
            or branch == keyword
        ):

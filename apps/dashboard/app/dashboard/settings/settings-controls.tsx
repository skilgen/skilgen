...
/**
 * Renders editable general and notification controls for org settings.
 */
export function SettingsControls({ accessToken, initialSettings, orgId, tab }: SettingsControlsProps): ReactElement {
  ...
  /**
   * Persists the current settings form to the API.
   */
  ...
  /**
   * Sends a Slack test notification through the API.
   */
    ...
    return (
      <section className="rounded-lg border border-[color:var(--bg-border)] bg-[color:var(--bg-surface)] p-6">
        <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
          ...
          <button
            className="inline-flex h-9 items-center rounded-md bg-[#C9973A] px-4 text-[13px] font-semibold text-black transition-colors hover:bg-[#d7aa55] disabled:cursor-wait disabled:opacity-60"
            disabled={saving}
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_260px]">
          <label className="block">
            ...
            <input
              className="mt-2 h-11 w-full rounded-md border border-[color:var(--bg-border)] bg-[#08080d] px-3 text-[14px] text-white outline-none transition-colors focus:border-[#C9973A]"
          ...
          <div>
            ...
            <input
              ...
              className="mt-4 w-full accent-[#C9973A]"
              max={100}
              min={0}
              onChange={(event) => setSettings((current) => ({ ...current, score_threshold: Number(event.target.value) }))}
              type="range"
    ...
    <section className="rounded-lg border border-[color:var(--bg-border)] bg-[color:var(--bg-surface)] p-6">
      <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          <button
            className="inline-flex h-9 items-center rounded-md border border-[#C9973A]/60 px-4 text-[13px] font-semibold text-[#C9973A] transition-colors hover:bg-[#C9973A]/10 disabled:cursor-wait disabled:opacity-60"
            disabled={testing || !slackValue}
      ...
      <label className="block">
        ...
        <input
          className="mt-2 h-11 w-full rounded-md border border-[color:var(--bg-border)] bg-[#08080d] px-3 font-mono text-[13px] text-white outline-none transition-colors focus:border-[#C9973A]"
          onChange={(event) => setSettings((current) => ({ ...current, slack_webhook_url: event.target.value }))}
...
/**
 * Renders a binary settings toggle.
 */
function Toggle({ checked, label, onChange }: ToggleProps): ReactElement {
  return (
    <button
      aria-pressed={checked}
      className="flex min-h-[56px] items-center justify-between rounded-lg border border-[color:var(--bg-border)] bg-[#08080d] px-4 text-left transition-colors hover:border-[#C9973A]/50"
...
/**
 * Renders a compact status notice for settings actions.
 */
function StatusNotice({ status }: { status: NonNullable<StatusState> }): ReactElement {
  return (
    <div
      className={
        status.tone === "success"
          ? "mt-5 flex items-center gap-2 rounded-md border border-[#3dd68c]/30 bg-[#3dd68c]/10 px-3 py-2 text-[13px] font-medium text-[#91efbd]"
          : "mt-5 rounded-md border border-[color:var(--accent-red)]/30 bg-[color:var(--accent-red)]/10 px-3 py-2 text-[13px] font-medium text-[color:var(--accent-red)]"
      }
    >
